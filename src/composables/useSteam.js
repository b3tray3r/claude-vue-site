...
const refreshAuth = async () => {
  const steamId = getSteamIdFromURL() || localStorage.getItem('steamId')
  if (steamId) {
    localStorage.setItem('steamId', steamId)
    const correctPath = getCorrectPath()
    if (window.location.search.includes('steamId')) {
...
// Один раз при загрузке приложения — инициализируем авторизацию
refreshAuth()
